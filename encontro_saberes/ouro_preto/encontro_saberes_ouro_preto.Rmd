...
library(here)
knitr::opts_knit$set(root.dir = here("encontro_saberes", "ouro_preto"))
```
...
# ler dados
ouro_preto <- read_csv("Planilha Masto Aves 2017 a 21 Resex Ouro Preto Validada.csv") 
...
tabela_1 <- ouro_preto %>%
  ...
  mutate(nome_popular = ifelse(especie == "Cavia fulgida", "Preá",
                        ...
                        ifelse(especie == "Ateles chamek", "Macaco-aranha",
                        ifelse(especie == "Saguinus weddelli", "Soim",
                        ifelse(especie == "Cebus unicolor", "Cairara",
                        ifelse(especie == "Saimiri ustus", "Mico-de-cheiro",
                        ifelse(especie == "Sapajus apella", "Macaco-prego",
                        ifelse(especie == "Callicebus brunneus", "Zogue-zogue",
                        ifelse(especie == "Pithecia irrorata", "Macaco-velho",
                        "no" ))))))))))))))))))))))) %>%
...
taxa_encontro_wide <- taxa_encontro_wide %>%
  mutate(média = round(rowMeans(.[,2:6]),3)) %>%
  arrange(desc(média)) %>%
  filter(média > 0.04) %>%
  select(-média) %>%
...
knitr::kable(
  tabela_esforco,
  caption = 'Tabela 1. Esforço de amostragem anual (em km) nas trilhas do Programa Monitora na Resex do Rio Ouro Preto, 2017-2021', booktabs = T, align = c("c", "l", "c")
  )
```
...
A tabela 2 apresenta o número total de encontros e de indivíduos avistados por espécie. As espécies mais encontradas foram, nessa ordem: macaco-prego, cutia, mico-de-cheiro, macaco-aranha e zogue-zogue. As outras espécies foram encontradas poucas vezes. Para as espécies que vivem em grupos, o número indivíduos pode ser maior do que o número de encontros.
...
knitr::kable(
  tabela_1,
  caption = 'Tabela 2. Numero de registros por especie obtidos pelo protocolo básico do Programa Monitora na Resex do Rio Ouro Preto, 2017-2021', booktabs = T, align = c("l", "l", "l", "l", "c")
  )
```
...
```{r, echo=FALSE, message=FALSE, warning=FALSE, fig.cap="Figura 1. Número total de encontros por espécie na Resex do Rio Ouro Preto, 2017-2021. Somente espécies com mais de 5 encontros foram incluídas"}
...
Somente o macaco-prego e cutia apresentaram taxa de encontro anual média > 0.04 ind/km, o que equivale aproximadamente a um encontro a cada 25 km percorridos (Tabela 3). A Figura 2 mostra que a taxa de avistamento para o macaco-prego diminuiu drasticamente ao longo do monitoramento. Já a taxa de avistamento para a cutia se manteve estável durante a maior parte do tempo, mas sofreu uma queda em 2021 (Figura 3).
...
knitr::kable(
  taxa_encontro_wide,
  caption = 'Tabela 3. Taxas de encontro anuais na Resex do Rio Ouro Preto', booktabs = T, align = c("l", "c")
  )
...
computar.plotar.taxa_encontro <- function(nome_especie) {
  ...
  plot_tendencia <- ggplot() + 
    ...
    coord_cartesian(ylim = c(0, max(modelo_quantis$quant75)+0.05) ) +
    xlab("Ano") + 
  ...
  # salvar jpeg
  #ggsave(paste(nome_especie, "2017 a 2021.jpeg", sep = " "))
  ...
  N_trilha3[1] <- NA # esforco zero no primeiro ano
  ...
  # salvar jpeg
  #ggsave(paste(nome_especie, "2017 a 2021 por trilha.jpeg", sep = " "))
...
```{r, echo=FALSE, message=FALSE, warning=FALSE, fig.cap="Figura 2. Taxa de encontro anual para macaco-prego na Resex do Rio Ouro Preto, 2017-2021"}
plot_tendencia_Sapajus_apella

```


```{r, echo=FALSE, message=FALSE, warning=FALSE, fig.cap="Figura 3. Taxa de encontro anual para cutia na Resex do Rio Ouro Preto, 2017-2021"}
...
A análise anterior avaliou as tendências médias, agregando os dados de todas a trilhas. No entanto, também é importante avaliar o que acontece em cada trilha separadamente. A figura 4 mostra que o macaco-prego declinou em todas as trilhas enquanto que as tendências para a cutia variaram bastante entre as trilhas. 


```{r, echo=FALSE, message=FALSE, warning=FALSE, fig.cap="Figura 4. Taxa de encontro anual para macaco-prego por trilha na Resex do Rio Ouro Preto, 2017-2021"}
plot_tendencia_trilha_Sapajus_apella

```


```{r, echo=FALSE, message=FALSE, warning=FALSE, fig.cap="Figura 5. Taxa de encontro anual para cutia por trilha na Resex do Rio Ouro Preto, 2017-2021"}
...
As espécies mais comuns encontradas no monitoramento foram macaco-prego, cutia, mico-de-cheiro, macaco-aranha e zogue-zogue. Isso está de acordo com a percepção da comunidade?

A tendência de queda para o macaco-prego está de acordo com a percepção da comunidade? Como explicar essa tendência? Está acontecendo alguma coisa na região que possa explicar isso?

Como explicar a estabilidade da cutia, e a queda em 2021?

Porque as tendências populacionais para as cutias variaram tanto entre as trilhas?
