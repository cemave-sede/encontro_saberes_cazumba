---
title: "Encontro dos Saberes PARNA do Jaú (2022)"
runningheader: "Análise mamíferos terrestres – 2015 a 2021" # only for pdf output
subtitle: "Análise mamíferos terrestres – 2015 a 2021" # only for html output
...
library(here)
knitr::opts_knit$set(root.dir = here("encontro_saberes", "jau"))
```
...
# ler dados
jau <- read_csv(here("encontro_saberes", "jau", "Planilha Oficial MastoAves Jau até 2021.csv") )
...
jau <- jau %>%
  ...
  mutate(nome_ea = replace(nome_ea, grep("ara", nome_ea), 
                           "Tiaraca"),
         nome_ea = replace(nome_ea, grep("ol", nome_ea), 
                           "Volta") )
jau
...
jau <- jau %>%
  filter(especie != "Primates") %>%
  mutate(especie = replace(especie, grep("Dasyprocta", especie), 
                           "Dasyprocta fuliginosa"),
         especie = replace(especie, especie %in% c("Guerlinguetus aestuans", "Urosciurus igniventris", "Microsciurus flaviventer"),
                                  "Sciuridae") )
...
tabela_esforco <- jau %>%
  ...
  mutate(`2020` = as.numeric(NA)) %>%
  select(EA, `nome EA`, `2015`, `2016`, `2017`, `2018`, `2019`, `2020`, `2021`)
...
tabela_1 <- jau %>%
  ...
  mutate(nome_popular = ifelse(especie == "Alouatta juara", "Guariba",
                        ifelse(especie == "Cacajao melanocephalus", "Uacarí-preto",
                        ifelse(especie == "Cebus albifrons", "Cairara",
                        ifelse(especie == "Choloepus didactylus", "Preguiça-real",
                        ifelse(especie == "Dasyprocta fuliginosa", "Cutia",
                        ifelse(especie == "Eira barbara", "Irara",
                        ifelse(especie == "Guerlinguetus aestuans", "Quatipuru",
                        ifelse(especie == "Leopardus pardalis", "Jaguatirica",
                        ifelse(especie == "Leopardus tigrinus", "Gato-do-mato",
                        ifelse(especie == "Lontra longicaudis", "Lontra",
                        ifelse(especie == "Mazama americana", "Veado-mateiro",
                        ifelse(especie == "Mazama nemorivaga", "Veado-fuboca",
                        ifelse(especie == "Microsciurus flaviventer", "Quatipuru",
                        ifelse(especie == "Myoprocta pratti", "Cutiara",
                        ifelse(especie == "Pecari tajacu", "Caititu",
                        ifelse(especie == "Pithecia pithecia", "Macaco-velho",
                        ifelse(especie == "Pteronura brasiliensis", "Ariranha",
                        ifelse(especie == "Puma concolor", "Onça-vermelha",
                        ifelse(especie == "Saimiri cassiquiarensis", "Mico-de-cheiro",
                        ifelse(especie == "Sapajus apella", "Macaco-prego",
                        ifelse(especie == "Tamandua tetradactyla", "Mambira",
                        ifelse(especie == "Tayassu pecari", "Queixada",
                        ifelse(especie == "Sapajus apella", "Macaco-prego",
                        ifelse(especie == "Urosciurus igniventris", "Quatipuru",
...
taxa_encontro_wide <- taxa_encontro %>%
  ...
  mutate(`2020` = as.numeric(NA)) %>%
  select(especie, `2015`, `2016`, `2017`,  `2018`, `2019`, `2020`, `2021`) %>%
  replace(is.na(.), 0.001) %>%
  mutate(`2020` = as.numeric(NA)) 

# selecionar as especies mais comuns (taxa media > 0.025)
taxa_encontro_wide <- taxa_encontro_wide %>%
  mutate(média = round(rowMeans(.[,2:8], na.rm=TRUE),3)) %>%
  arrange(desc(média)) %>%
  filter(média >= 0.04) %>%
  #select(-média) %>%
...
Este relatório apresenta resultados do Programa Monitora, Subprograma Terrestre, Componente Florestal Global, Alvo Mamíferos terrestres de médio e grande porte, protocolo básico, na **Parque Nacional do Jaú**, 2015 a 2021. O principal objetivo é subsidiar o Encontro dos Saberes que será realizado nesta unidade.


## Métodos

O protocolo básico para mamíferos utiliza o método de transecções lineares. Resumidamente, dois observadores percorrem as transecções a uma velocidade constante e registram, para cada avistamento de animais dos grupos-alvo, a espécie, o número de indivíduos e a distância perperndicular entre o primeiro indivíduo avistado e o centro da trilha. Cada UC deve ter pelo menos três transectos de 5 km e o esforço anual mínimo, combinando todos os transectos, deve ser de 150 km percorridos. A tabela 1 apresenta o esforço anual por trilha na PARNA do jau.
...
knitr::kable(
  tabela_esforco,
  caption = 'Tabela 1. Esforço de amostragem anual (em km) nas trilhas do Programa Monitora na PARNA do Jaú, 2015-2021', booktabs = T, align = c("c", "l", "c")
  )
```
...
A tabela 2 apresenta o número total de encontros e de indivíduos avistados por espécie. As espécies mais encontradas foram, nessa ordem: macaco-prego, quatipuru, cutiara, macaco-velho e cutia. A maioria das outras espécies foram encontradas poucas (< 10) vezes. Para as espécies que vivem em grupos, o número indivíduos pode ser maior do que o número de encontros.
...
knitr::kable(
  tabela_1,
  caption = 'Tabela 2. Numero de registros por especie obtidos pelo protocolo básico do Programa Monitora na PARNA do Jaú, 2015-2021', booktabs = T, align = c("l", "l", "l", "l", "c")
  )
```
...
```{r, echo=FALSE, message=FALSE, warning=FALSE, fig.cap="Figura 1. Número total de encontros por espécie na PARNA do Jaú, 2015-2021. Somente espécies com mais de 5 encontros foram incluídas"}
...
Somente cinco espécies apresentaram taxa de encontro anual média > 0.04 ind/km, o que equivale aproximadamente a um encontro a cada 25 km percorridos (Tabela 3): macaco-prego, quatipuru (três espécies agrupadas), cutiara, macaco-velho e cutia. As Figuras 2 a 5 mostram como as taxas de avistamento variaram para estas espécies ao longo do monitoramento.

A taxa de avistamento de cutia, macaco-aranha e soim parece estar aumentando levemente ao longo do tempo. Por outro lado, a taxa de avistamento do macaco-prego variou bastante ao longo do tempo, com um aumento entre 2016 e 2019, uma queda em 2020 e uma tendência de recuperação em 2021.
...
knitr::kable(
  taxa_encontro_wide,
  caption = 'Tabela 3. Taxas de encontro anuais no PARNA do Jaú', booktabs = T, align = c("l", "c")
  )
...
state.space.model <- function(y) {
  ...
  sink(here("encontro_saberes", "jau", "ssm.jags"))
  cat("
  model {
  ...
  ssm <- jags(jags.data, inits=NULL, parameters, here("encontro_saberes", "jau", "ssm.jags"), n.chains = nc, n.thin = nt, n.iter = ni, n.burnin = nb)
...
computar.plotar.taxa_encontro <- function(nome_especie) {
  ...
  N.est_por_trilha <- as_tibble(rbind(N_trilha1,
                                      N_trilha2,
                                      N_trilha3)) %>%
    rename('2015' = V1, '2016' = V2, '2017' = V3,
           '2018' = V4, '2019' = V5, '2020' = V6, '2021' = V7) %>%
    mutate('2020'= NA) %>%
    mutate(trilha = c("Volta", "Cachoeira", "Tiaraca")) %>%
...
computar.plotar.taxa_encontro("Sapajus apella")
...
computar.plotar.taxa_encontro("Pithecia pithecia")
...
```{r, echo=FALSE, message=FALSE, warning=FALSE, fig.cap="Figura 2. Taxa de encontro anual para macaco-prego na PARNA do Jaú, 2015-2021"}
plot_tendencia_Sapajus_apella

```


```{r, echo=FALSE, message=FALSE, warning=FALSE, fig.cap="Figura 3. Taxa de encontro anual para macaco-velho na PARNA do Jaú, 2015-2021"}
plot_tendencia_Pithecia_pithecia

```



```{r, echo=FALSE, message=FALSE, warning=FALSE, fig.cap="Figura 4. Taxa de encontro anual para cutia na PARNA do Jaú, 2015-2021"}
plot_tendencia_Dasyprocta_fuliginosa

```


```{r, echo=FALSE, message=FALSE, warning=FALSE, fig.cap="Figura 5. Taxa de encontro anual para cutiara na PARNA do Jaú, 2015-2021"}
plot_tendencia_Myoprocta_pratti

```


```{r, echo=FALSE, message=FALSE, warning=FALSE, fig.cap="Figura 6. Taxa de encontro anual para quatipuru na PARNA do jau, 2015-2021"}
...
A análise anterior avaliou as tendências médias, agregando os dados de todas a trilhas. No entanto, também é importante avaliar o que acontece em cada trilha separadamente. As figuras 6 a 9 mostram que as trilhas apresentam tendências razoavelmente parecidas, apesar de faltarem dados para algumas trilhas em alguns anos e haver certa variação entre as trilhas. 


```{r, echo=FALSE, message=FALSE, warning=FALSE, fig.cap="Figura 7. Taxa de encontro anual para macaco-prego por trilha na PARNA do Jaú, 2015-2021"}
plot_tendencia_trilha_Sapajus_apella

```


```{r, echo=FALSE, message=FALSE, warning=FALSE, fig.cap="Figura 8. Taxa de encontro anual para macaco-velho por trilha na PARNA do Jaú, 2015-2021"}
plot_tendencia_trilha_Pithecia_pithecia

```



```{r, echo=FALSE, message=FALSE, warning=FALSE, fig.cap="Figura 9. Taxa de encontro anual para cutia por trilha na PARNA do Jaú, 2015-2021"}
plot_tendencia_trilha_Dasyprocta_fuliginosa

```


```{r, echo=FALSE, message=FALSE, warning=FALSE, fig.cap="Figura 10. Taxa de encontro anual para macaco-prego por trilha na PARNA do Jaú, 2015-2021"}
plot_tendencia_trilha_Myoprocta_pratti

```


```{r, echo=FALSE, message=FALSE, warning=FALSE, fig.cap="Figura 11. Taxa de encontro anual para macaco-aranha por trilha na PARNA do Jaú, 2015-2021"}
#plot_tendencia_trilha_Sciuridaae
...
As espécies mais comuns encontradas no monitoramento foram macaco-prego, quatipuru, cutiara, macaco-velho e cutia. Isso está de acordo com a percepção da comunidade?

Como explicar a tendência de aumento na taxa de encontro de cutias, macaco aranha e soim? Está acontecendo alguma coisa na região que possa explicar isso?

O aumento para as três espécies parece ter sido mais forte em 2021. Isso se deve a algum fator natural, ou será que houve alguma mudança na equipe responsável pelo monitoramento, ou nos métodos utilizados? Será que a equipe está ficando mais treinada?

Como explicar as variações na taxa de avistamento de macacos-prego?

Como explicar as variações na taxa de avistamento entre as trilhas?
